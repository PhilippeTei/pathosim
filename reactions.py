from . import utils as cvu
    ...
    # Ritchie's stuff. 
    # First define categories of people that might want to get tested.
    # test_cats = self.categorise_people() # a dictionary
    # pot_rat_ppl = self.rat_pot_seekers(test_cats)
    # pot_pcr_ppl = self.pcr_pot_seekers(test_cats)

    # # Assume preference for RAT in the case of overlap.  
    # rat_ppl = self.rat_seekers()
    # pcr_ppl = self.pcr_seekers(rat_ppl) # if already taken by rat criterion, don't add here. 

    # # do the testing. 
    # rat_tests(rat_ppl) # determines results and timing.
    # pcr_tests(pcr_ppl) # determines results and timing.

# Store in a class so we can store state. 
# Absorbs SwBehaviour.
# Also absorbs PCR and RAT classes? 
# Alternatively, could call these classes, so the classes can keep up their logging?


class Reactions:
    """
    OBJECTIVE: Model how people react to their conditions.
    It separately captures the decision to take tests and other behavioural decisions.
    SKELE: 
    - Schedule_tests: decides who takes a test and when they get the results. 
    - Update_behaviour: decides who goes to quarantine.
    TASKS:
    - Update Schedule_tests with additional thinking. 
    """
    def __init__():
        """
        TODO: sw_behav_params, etc. 
        """
        
        pass
    def schedule_tests(self, sim):
        """
        people: Covasim People object. 
        """
        people_cat_dict = ReactionCategories(sim.people).cat_dict
        # Pipe some to RAT. 
        


    def update_behaviour(people):
        pass


class ReactionCategories:
    """
    ...
    """
    def __init__(self, surv, sim): 
        self.criteria = ['pos_test', 
                         'cs_sx', 
                         'sw_alert']

        self.cat_dict = self.check_criteria(surv, sim)
    ...
    def testers(self, sim, mode='pos'):
        ...
        raise NotImplementedError('testers() is not yet implemented')
