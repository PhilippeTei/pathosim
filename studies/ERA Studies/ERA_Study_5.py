...
output_directory = "c:\\Outputs\\Simulations\\Results\\Replication_Main\\Study_5"
...
#Threat Net surveillance setup
threat_net_pars = base_pars.copy()
threat_net_pars['enable_surveillance'] = True
threat_net_pars['enable_syndromic_testing'] = True
threat_net_pars['syndromic_pooling_enabled'] = True
threat_net_pars['syndromic_pool_size'] = 6
...
rand_pars['read_count'] = 40000000 #number of reads per sample

# Ideal surveillance setup
ideal_pars = base_pars.copy()
ideal_pars['enable_surveillance'] = True
ideal_pars['surveillance_test_percent'] = 0.002 
ideal_pars['enable_severity_testing'] = True
ideal_pars['severity_symp_prob_threshold'] = 0.75
ideal_pars['severity_test_frequency'] = 1
ideal_pars['pooling_enabled'] = True
ideal_pars['pool_size'] = 10
ideal_pars['read_count'] = 10000000 #number of reads per sample


def generate_label(pars):

    # Generate labels for the three simulation setups
    if pars == threat_net_pars:
        label = 'Threat_Net'
    elif pars == rand_pars:
        label = 'Random_Testing'
    elif pars == ideal_pars:
        label = 'Ideal_Testing'
...
# Run the simulations
ideal_results = run_simulation(ideal_pars, loops=100)
threat_net_results = run_simulation(threat_net_pars, loops=100)
rand_results = run_simulation(rand_pars, loops=100)

#append results to dataframe
threat_net_results_df = pd.concat([pd.DataFrame([threat_net_results], columns=results_df.columns)], ignore_index=True)
rand_results_df = pd.concat([pd.DataFrame([rand_results], columns=results_df.columns)], ignore_index=True)
ideal_results_df = pd.concat([pd.DataFrame([ideal_results], columns=results_df.columns)], ignore_index=True)

#export results to csv
results_csv_path = os.path.join(output_directory, "final_cumulative_results.csv")
results_df = pd.concat([threat_net_results_df, rand_results_df, ideal_results_df], ignore_index=True)
