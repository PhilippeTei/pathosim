...
output_directory = "c:\\Outputs\\Simulations\\Results\\Replication_Main\\Study_3"
...
rand_pars['read_count'] = None # Placeholder
rand_pars['hospital']

# Define Iteration Lists:   
rand_pars_read_count_values = [1000000, 2000000, 5000000, 10000000, 20000000, 40000000]

def generate_label(pars):
    
    rand_pars_read_count = pars.get('read_count', None)

    # Generate a label for the simulation setup based on the parameters
    label = f"Read_Count_{rand_pars_read_count}"
...
# Run the simulations
for read_count in rand_pars_read_count_values:
    rand_pars['read_count'] = read_count
    result = run_simulation(rand_pars, 50)
    result_df = pd.concat([pd.DataFrame([result], columns=results_df.columns)], ignore_index=True)

#export to csv
results_csv_path = os.path.join(output_directory, "cumulative_results.csv")
results_df.to_csv(results_csv_path, index=False)

#TOTAL: 6 simulations; 100 iterations each; 600 simulations total
